...
def run_backtest(data, strategy):
    ...
    for i in range(20, len(data)):
        ...
        entry, stop, take, _, _, _, _, _, _ = generate_signals(slice_data, strategy)
        signals.append((entry, stop, take))
    wins = sum(1 for e, s, t in signals if t > e)
    losses = sum(1 for e, s, t in signals if s < e)
    if wins + losses == 0:
        return "No valid backtest signals."
    return f"Win Rate: {wins/(wins+losses):.2%} from {len(signals)} trades"
...
if trigger:
    ...
    if data.empty:
        ...
        st.session_state.trade_history.append({
            'Time':timestamp,'Ticker':ticker,'Strat':strategy,
            'Signal':'SELL' if sell_signal else 'BUY','Entry':entry,
            'Stop':stop,'TP':take,'Shares':shares,'Conf':confidence,
            'Reason':reason,'Sent':sentiment,'Status':'Open'
        })
        # plot
